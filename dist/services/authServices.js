...
export const AuthService = {
    async register({ username, email, password, userRole, profile_image }) {
        ...
        const verificationLink = `http://192.168.208.248:4000/api/auth/verify-email?token=${verificationToken}`;
        const emailHtml = `
  ...
  `;
    ...
    async getProfile(requestingUserId, requestingUserRole, targetId) {
        const userRepository = AppDataSource.getRepository(User);
    ...
    async forgotPassword(email) {
        ...
        const resetLink = `http://192.168.208.248:4000/api/auth/reset-password?token=${resetToken}`;
        const emailHtml = `
    ...
    async editUser(adminRole, targetUserId, updates) {
    ...
    async deleteUser(adminRole, targetUserId) {
        if (adminRole !== UserRole.ADMIN)
